...
mod tests {
    ...
    fn tests_substitute_raw_pkh() {
        ...
        // Get PSBT from its hex and deserialize it instead of this dummy object.

        // Create a new function to put things inside of hash_map instead of this bulky logic
        let mut hash_map: HashMap<hash160::Hash, PublicKey> = HashMap::new();
        let psbt_inputs = &psbt.inputs;
        ...
        if let Some(ref witness_script) = &psbt.inputs[0].witness_script {
            let ms = Miniscript::<bitcoin::PublicKey, Segwitv0>::parse_with_ext(
                witness_script,
                &ExtParams::allow_all(),
            )
            .unwrap();
            ms.substitute_raw_pkh(&hash_map);
            // Assert Eq ms here and expected ms.
        }
    }
}
