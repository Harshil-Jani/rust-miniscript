...
impl<Pk: MiniscriptKey, Ctx: ScriptContext> Terminal<Pk, Ctx> {
    ...
    pub fn substitute_raw_pkh(&mut self, pk_map: &HashMap<hash160::Hash, Pk>) {
        match self {
            Terminal::RawPkH(ref mut p) => {
                if let Some(pk) = pk_map.get(p) {
                    *self = Terminal::PkH(pk.clone()).into();
                }
            }
            Terminal::PkK(..)
            | Terminal::PkH(..)
            ...
            | Terminal::Hash160(..)
            | Terminal::True
            | Terminal::False => *self = Terminal::False,
            Terminal::Alt(ref mut sub) => {
                sub.substitute_raw_pkh(pk_map);
            }
            Terminal::Swap(ref mut sub) => {
                sub.substitute_raw_pkh(pk_map);
            }
            Terminal::Check(ref mut sub) => {
                sub.substitute_raw_pkh(pk_map);
            }
            Terminal::DupIf(ref mut sub) => {
                sub.substitute_raw_pkh(pk_map);
            }
            Terminal::Verify(ref mut sub) => {
                sub.substitute_raw_pkh(pk_map);
            }
            Terminal::NonZero(ref mut sub) => {
                sub.substitute_raw_pkh(pk_map);
            }
            Terminal::ZeroNotEqual(ref mut sub) => {
                sub.substitute_raw_pkh(pk_map);
            }
            Terminal::AndV(ref mut left, ref mut right) => {
                left.substitute_raw_pkh(pk_map);
                right.substitute_raw_pkh(pk_map);
            }
            Terminal::AndB(ref mut left, ref mut right) => {
                left.substitute_raw_pkh(pk_map);
                right.substitute_raw_pkh(pk_map);
            }
            Terminal::AndOr(ref mut a, ref mut b, ref mut c) => {
                a.substitute_raw_pkh(pk_map);
                b.substitute_raw_pkh(pk_map);
                c.substitute_raw_pkh(pk_map);
            }
            Terminal::OrB(ref mut left, ref mut right) => {
                left.substitute_raw_pkh(pk_map);
                right.substitute_raw_pkh(pk_map);
            }
            Terminal::OrD(ref mut left, ref mut right) => {
                left.substitute_raw_pkh(pk_map);
                right.substitute_raw_pkh(pk_map);
            }
            Terminal::OrC(ref mut left, ref mut right) => {
                left.substitute_raw_pkh(pk_map);
                right.substitute_raw_pkh(pk_map);
            }
            Terminal::OrI(ref mut left, ref mut right) => {
                left.substitute_raw_pkh(pk_map);
                right.substitute_raw_pkh(pk_map);
            }
            Terminal::Thresh(k, ref mut subs) => {
                let subs: Vec<Arc<Miniscript<_, _>>> = subs
                    .iter()
                    .map(|s| Arc::new(s.substitute_raw_pkh(pk_map)))
                    .collect();
                *self = Terminal::Thresh(*k, subs);
            }
        }
    }
}
